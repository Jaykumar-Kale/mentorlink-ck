      ...
      <header className="bg-white border-b border-gray-100 shadow-card sticky top-0 z-40">
        <div className="flex items-center justify-between px-4 sm:px-6 h-16">
          <Link to="/" className="flex items-center gap-3">
            ...
            <div className="hidden sm:block">
              <p
                className="font-black text-ck-dark text-sm"
            ...
            {navItems.map(({ to, label, icon }) => (
              <Link
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all ${
                  pathname === to
                    ? "bg-ck-blue text-white shadow-sm"
                    : "text-ck-text hover:text-ck-blue hover:bg-ck-blue-l"
                }`}
              >
          ...
          <div className="flex items-center gap-3">
            <img
              src={avatar}
              alt="avatar"
              className="w-9 h-9 rounded-xl object-cover border-2 border-ck-blue-l"
            />
            <div className="hidden sm:block">
              <p className="text-sm font-bold text-ck-dark leading-tight">
                {user?.name}
              </p>
              <p className="text-xs text-ck-orange font-bold capitalize">
                {user?.role}
              </p>
            </div>
            <button
              onClick={handleLogout}
              className="hidden sm:block text-xs text-ck-muted hover:text-red-500 font-semibold transition px-2"
            >
              ↩
            </button>
            <button
              onClick={() => setMobileOpen(!mobileOpen)}
              className="md:hidden p-2 rounded-xl hover:bg-ck-gray"
            >
              ☰
            </button>
          </div>
        ...
        {mobileOpen && (
          <div className="md:hidden bg-white border-t border-gray-100 px-4 py-3 flex flex-wrap gap-2">
            ...
            <button
              onClick={handleLogout}
              className="px-3 py-2 rounded-xl text-xs font-bold text-red-500 bg-red-50"
            >
              Sign Out
            </button>
          </div>
